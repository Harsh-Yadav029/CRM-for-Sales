  ...
  return (
    <RoleGate allow={['admin']}>
      <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto pb-24 md:pb-6">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Developer Center</h2>
          <p className="text-xs text-slate-400">Manage tenant-scoped API keys and test programmatic webhook calls</p>
        </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 backdrop-blur-sm space-y-4">
              ...
              <form onSubmit={handleGenerate} className="flex gap-3">
                <input
                  ...
                  className="flex-1 bg-slate-950 border border-slate-850 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-amber-500 transition-colors"
                />
                <button
                  type="submit"
                  disabled={genLoading || !keyName.trim()}
                  className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-xl flex items-center gap-1 transition-all disabled:opacity-50 shrink-0"
                >
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 backdrop-blur-sm">
              {loading ? (
                ...
                <p className="text-xs text-slate-500 italic text-center py-8">No active API keys found</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-slate-800 bg-slate-950/40 text-slate-500 font-bold uppercase text-[10px]">
                        ...
                        return (
                          <tr key={k._id} className="text-slate-300">
                            <td className="py-3 px-3">
                              ...
                              <p className="text-[9px] text-slate-500 mt-0.5">Created: {new Date(k.createdAt).toLocaleDateString()}</p>
                            </td>
                            <td className="py-3 px-3">
                              <div className="flex items-center gap-2">
                                <span className="font-mono bg-slate-950 px-2 py-1 rounded text-[10px] select-all border border-slate-850">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 backdrop-blur-sm space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <pre className="bg-slate-950 p-3 rounded-xl border border-slate-850 text-[10px] font-mono text-amber-400 overflow-x-auto max-h-36">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <pre className="bg-slate-950 p-3 rounded-xl border border-slate-850 text-[10px] font-mono text-emerald-400 overflow-x-auto max-h-36">
